...
from PyQt5.QtWidgets import (QApplication, QWidget, QPushButton, QLabel,
                             QVBoxLayout, QHBoxLayout, QLineEdit, QListWidget, QInputDialog, QMessageBox, QListWidgetItem, QDialog , QComboBox, QDialogButtonBox, QFormLayout , QSpinBox)
...
class EditAnimeWindow(QDialog):
    ...
    def __init__(self, anime_name, anime_data, neo_anime_data):
        super().__init__()

        self.old_name = anime_name
        self.neo_anime_data = neo_anime_data
        ...
        self.name_edit = QLineEdit(anime_name)
    ...
    def save_changes(self):
        if self.old_name in self.neo_anime_data:
                del self.neo_anime_data[self.old_name]
        # Update the anime data with the changes
        self.neo_anime_data[self.name_edit.text()] = {
            "name":self.name_edit.text(),
            "page": self.page_edit.text(),
            "tag": self.tag_edit.text(),
            "episodes_downloaded": int(self.episodes_edit.text()),  # subtract 1 to account for 0-indexing
            "resolution": self.resolution_combo.currentText()
        }

        # self.anime_data["name"] = self.name_edit.text()
        ...
        self.anime_edited.emit(self.neo_anime_data)
        self.accept()
...
class AnimeMagnetDownloader(QWidget):
    ...
    def edit_anime(self):
        ...
        neo_anime_data = self.anime_list

        # Create a new window for editing the anime
        dialog = EditAnimeWindow(anime_name, anime_data, neo_anime_data)
        if dialog.exec_() == QDialog.Accepted:
            # Update the anime data with the changes
            self.anime_list = neo_anime_data
    ...
    def add_anime(self):
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("Add Anime")
        layout = QFormLayout(dialog)
        anime_name_edit = QLineEdit()
        anime_page_edit = QLineEdit()
        anime_tag_edit = QLineEdit()
        starting_episode_spinbox = QSpinBox()
        starting_episode_spinbox.setRange(1, 9999)
        layout.addRow(QLabel("Name:"), anime_name_edit)
        layout.addRow(QLabel("Page:"), anime_page_edit)
        layout.addRow(QLabel("Tag:"), anime_tag_edit)
        layout.addRow(QLabel("Starting Episode Number:"), starting_episode_spinbox)
        resolution_combo = QComboBox()
        resolution_combo.addItem("1080p")
        resolution_combo.addItem("720p")
        resolution_combo.addItem("480p")
        layout.addRow(QLabel("Resolution:"), resolution_combo)
        ...
        if dialog.exec_() == QDialog.Accepted:
            anime_name = anime_name_edit.text()
            anime_page = anime_page_edit.text()
            anime_tag = anime_tag_edit.text()
            starting_episode = starting_episode_spinbox.value()
    ...
    def download(self):
        ...
        for anime in anime_list:
            ...
            while True:
                ...
                for div in divs:
                    ...
                    try:
                        ...
                        # match = regex.search(r"\[.*?\]\s*(?:.*?\s*-\s*)?" + anime.replace(".", r"\.") + r"(?:\s*\[.*?\])*\s*-?\s*(?:\[[^\]]*\]\s*){0,2}(?:\([^\)]*\)\s*){0,2}(?:\S+\s*){0,10}(\d{2,4})(?=[\s-])(?![^\[]*\])(?![^\(]*\))", mod_title)
                        # match = regex.search(r"\[.*?\]\s*(?:.*?\s*-\s*)?" + anime.replace(".", r"\.") + r"(?:\s*\[.*?\])*\s*-?\s*(?:\[[^\]]*\]\s*){0,2}(?:\([^\)]*\)\s*){0,2}(?:\S+\s*){0,10}(?<!\d)(\d{2,4})(?=[\s:-])(?![^\[]*\])(?![^\(]*\))", mod_title)
                        match = regex.search(r"\[.*?\]\s*(?:.*?\s*-\s*)?" + anime.replace(".", r"\.") + r"(?:\s*\[.*?\])*\s*-?\s*(?:\[[^\]]*\]\s*){0,2}(?:\([^\)]*\)\s*){0,2}(?:\S+\s*){0,10}(?<=\s)(\d{2,4})(?=[\s-])(?![^\[]*\])(?![^\(]*\))", mod_title)
                    except NameError:
            ...
            message = "\n".join(f"{title}" for title, magnet_link in episodes)
            added_anime = added_anime + message + "\n\n" 
            # QMessageBox.information(self, "Episodes", added_anime)
            print("\n", message, "\n")
        ...
        if(len(added_anime) > 0):
            print('All added episodes: ', "\n" ,added_anime)
